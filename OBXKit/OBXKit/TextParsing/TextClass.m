...
- (id)initWithText:(NSString *)theText withFont:(Font *)theFont andScreenSize:(CGSize)size
{
    ...
    if (self != nil)
    {
        ...
        {
            [temp setString:@""];
            for(Word *words1 in sentences.theWordObjects){
                [temp appendString:words1.currentWord];
            }
            NSLog(@"Text loaded is  %@", temp);
        }
         */
...
- (void) giveFocusFor:(int)sentenceID and:(int)wordID;
{
    ...
    //NSLog(@"New focus is %@", currSentence.theWordObjects);
    //for(Word *words1 in currSentence.theWordObjects)
     //   NSLog(@"New focus on currsentence: %@", words1.currentWord);
    /*
    NSMutableString *temp=[[NSMutableString alloc] init];;
    for(Word *words1 in currSentence.theWordObjects){
        [temp appendString:words1.currentWord];

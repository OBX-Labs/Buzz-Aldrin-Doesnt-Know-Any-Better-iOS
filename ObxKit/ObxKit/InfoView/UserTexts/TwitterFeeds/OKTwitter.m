...
//Note: VALID_CHAR_SET is changed because not all characters are set in the Know fonts.
//static NSString *VALID_CHAR_SET = @"!\"&'*,-.:?ABCDEFGHIKLMNOPRSTUVWXYabcdefghijklmnopqrstuvwxyzáèéêíîñóô12347 ";
...
{
    /*
    for(int i = 0; i < [VALID_CHAR_SET length]; i++)
    {
        NSString *character = [NSString stringWithFormat:@"%c", [VALID_CHAR_SET characterAtIndex:i]];
        NSLog(@"Log character: %@",  character);
        string= [string stringByReplacingOccurrencesOfString:character withString:@""];
        NSLog(@"Log new string: %@",  string);
    }
     */
    ...
    BOOL valid = [string isEqualToString:filteredText];

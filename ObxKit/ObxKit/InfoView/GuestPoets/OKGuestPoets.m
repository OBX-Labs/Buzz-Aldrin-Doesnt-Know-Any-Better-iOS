...
- (void) buildHeadersForTableView
{
    ...
    for(int i = 0; i < [[OKTextManager sharedInstance] packagesCount]; i++)
    {
        ...
        if(![[packages allKeys] containsObject:[textDict objectForKey:@"Author"]]) // Has not yet been created
        {
            ...
            if(![[NSFileManager defaultManager] fileExistsAtPath:imagePath] || [[textDict objectForKey:@"AuthorImage"] isEqualToString:@""] || [textDict objectForKey:@"AuthorImage"] == nil) imagePath = [[NSBundle mainBundle] pathForResource:@"placeholder" ofType:@"png"];

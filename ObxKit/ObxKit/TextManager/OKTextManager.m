...
- (BOOL) setTextList:(NSDictionary *)aDict andSave:(BOOL)aSave
{
    ...
    for(NSString *packageKey in packageKeys)
    {
        ...
        for(NSDictionary *textDict in package)
        {
            ...
            if(!imageFileExists && ![imageFile isEqualToString:@""]) {
...
- (BOOL) saveCustomText:(NSString*)text forTitle:(NSString*)title
{
    ...
    NSString *package = [[NSString alloc] initWithFormat:@"net.obxlabs.%@.%@.custom", [[OKAppProperties objectForKey:@"Name"] lowercaseString], deviceName];
...
- (BOOL) loadCustomTextFromPackage:(NSString *)package
{
    ...
    if(textDict == nil)
    {
        NSLog(@"loadCustomTextFromPackage - No textDict");
        return NO;
    }
    ...
    if(![[NSFileManager defaultManager] fileExistsAtPath:textPath])
    {
        NSLog(@"loadCustomTextFromPackage - No text file at path %@", textPath);
        return NO;
    }
    ...
    if(properties == nil)
    {
        NSLog(@"loadCustomTextFromPackage - No properties");
        return NO;
    }
    ...
    if(fontFile == nil)
    {
        NSLog(@"loadCustomTextFromPackage - No font file");
        return NO;
    }
